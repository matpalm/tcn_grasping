...
parser.add_argument('--frame-inc', type=int, default=1)
...
B=5  # pixel buffer
collage = Image.new('RGB', ((W+B)*num_cols, ((H+B)*num_rows)), (0,0,0))
...
for frame_offset in range(opts.num_frames):
    for row, camera_id in enumerate(camera_ids):
        fname = "/".join([opts.img_dir,
                          ...
                          u.frame_filename_format(opts.initial_frame + (frame_offset * opts.frame_inc))
                         ])
        img = Image.open(fname)
        collage.paste(img, ((W+B)*frame_offset, (H+B)*row))
